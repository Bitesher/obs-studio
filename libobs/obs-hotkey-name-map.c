...
struct obs_hotkey_name_map {
	...
	obs_hotkey_name_map_node_t *leafs;
	size_t num_leafs;
	size_t next_leaf;
};
...
static inline obs_hotkey_name_map_node_t *new_leaf(void)
{
	...
	obs_hotkey_name_map_node_t *node =
		&name_map->leafs[name_map->next_leaf];
...
#define OBS_HOTKEY(x) name_map->num_leafs += 1;
#include "obs-hotkeys.h"
#undef OBS_HOTKEY

	size_t size = sizeof(obs_hotkey_name_map_node_t) * name_map->num_leafs;
	name_map->leafs = bzalloc(size);
...
void obs_hotkey_name_map_free(void)
{
	...
	bfree(obs->hotkeys.name_map->leafs);
